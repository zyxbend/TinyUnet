...
from modeling.Unet_fixed import Unet
...
class Tester(object):
    ...
    def test(self, epoch):
        ...
        for i, batch in enumerate(tbar):
            image, target, img_name = batch[0], batch[1], batch[2]
            if self.args.cuda:
            ...
            pred_over_layed = combine(gt_img, pred_img, red, orange, blue, in_img, white)
...
def main():
    parser = argparse.ArgumentParser(description="PyTorch Testing NIH_dataset")

    parser.add_argument('--out-stride', type=int, default=16,
                        help='network output stride (default: 16)')
    parser.add_argument('--dataset', type=str, default='Nih_dataset',
                        choices=['EM_dataset', 'Nih_dataset', 'Tooth'],
                        help='dataset name (default: Nih_dataset)')
